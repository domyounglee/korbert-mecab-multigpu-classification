...
class EmotionClassifierProcessor(DataProcessor):
  def get_train_examples(self, data_dir):
    """See base class."""
    return self._create_examples(
        self._read_tsv(os.path.join(data_dir, "train.tsv")), "train")
  ...
  def get_labels(self):
    """See base class."""
    return ["놀라다/황당하다", "화나다", "슬프다", "두렵다", "수치스럽다","기쁘다"]
  ...
  def _create_examples(self, lines, set_type):
    ...
    for (i, line) in enumerate(lines):
      ...
      if i<10:
          tf.logging.info("===================="+line[0])


      guid = "%s-%s" % (set_type, i)
      if set_type == "test":
        text_a = tokenization.convert_to_unicode(line[0])
        label = "놀라다/황당하다" #this is useless label 
      else:
        text_a = tokenization.convert_to_unicode(line[0])
        label = tokenization.convert_to_unicode(line[1])
      examples.append(
...
def convert_single_example(ex_index, example, label_list, max_seq_length,
  ...
  paragraph_morp = do_lang(example.text_a)
  p_json = json.loads(paragraph_morp)['sentences']
  morp_tok = get_morphs(p_json)
  tokens_a = []
  for tok in morp_tok:
    tokens_a.extend(tokenizer.tokenize(tok))
  ######
...
def main(_):
  ...
  processors = {
      ...
      #"mrpc": MrpcProcessor,
      #"xnli": XnliProcessor,
      "emot": EmotionClassifierProcessor,
  }
